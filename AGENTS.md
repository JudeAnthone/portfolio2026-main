...
- **About bento grid** (`About.tsx`): outer grid is `grid-cols-2 gap-4 md:grid-cols-12 md:gap-5`. On `md+` the rows are: full-width hero narrative card (`md:col-span-12`, no chip/label, just story text) → 6 tech cells (`md:col-span-4`) → CTA card (`md:col-span-6`) + 2 metric cells (`md:col-span-3`) → Current Internship (`md:col-span-12`) → Gallery (`md:col-span-12`). On mobile the same grid is 2 columns (`col-span-1` for tech/metric cells, `col-span-2` for full-width cards).
- **Brand icons**: tech logos use `react-icons/si` (e.g. `SiReact`, `SiDocker`). For brands missing from react-icons (e.g. n8n), define a small inline `<svg fill="currentColor">` component in the same file (see `N8nIcon` in `About.tsx`).
